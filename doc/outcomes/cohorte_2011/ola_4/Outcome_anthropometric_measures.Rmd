...
bibliography: ../../../../www/BMI_bib.bib 
csl: "`r ecs.data::get_proj_csl_file()`"
nocite: '@world_health_organization_obesity_nodate, @cdc_defining_2022'
editor_options: 
  markdown:
    wrap: 80
params:
...
|                | cms (c2011_w4)          |                |                |
+----------------+-------------------------+----------------+----------------+
| bmi_s3         | Body mass index,        | Continuous     |                |
|                | continuous (c2011_w4)   |                |                |
+----------------+-------------------------+----------------+----------------+
| bmi_cat_s3     | Body mass index,        | Continuous     | 1 =            |
...
-   **Height** is renamed from q2506_height_s3 to height_s3. It is obtained by
    using a measuring tape. Constraints are set to allow only valid values.

-   **Weight** is renamed from q2507_weight to weight_s3. It is obtained using a
    weight scale without shoes. Constraints are set to allow only valid values.

-   **Waist circumference** is ranamed from q2508_waist_s3 to waist_s3. It is
    obtained by identifying the midpoint between the last rib and the illiac
...
-   **BMI**: BMI is defined as the weight in kilograms divided by the square of
    the height in meters (kg/m2) @world_health_organization_obesity_nodate,
    @cdc_defining_2022 ). Four categories are created using the WHO BMI cut-off
    points:
...
gen bmi_s3=.
replace bmi_s3= weight_s3/(height_s3*height_s3) if weight_s3>=27.3 & weight_s3<=200
...
label variable bmi_cat_s3 "Categories of BMI"
