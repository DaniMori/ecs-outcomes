...
| name**           |                  |                  |                  |
+:=================+:=================+==================+==================+
| d_life           | Severity of      | Ordinal          | -   0=No;        |
| time_severity_s3 | lifetime         |                  | -   1=Mild;      |
|                  | depressive       |                  | -   2=Moderate;  |
|                  | episode          |                  | -   3=Severe     |
|                  | (c2011_w4)       |                  |                  |
+------------------+------------------+------------------+------------------+
| depres           | Lifetime         | Binary           | -   0=no;        |
| sion_lifetime_s3 | depression       |                  | -   1=yes        |
|                  | (c2011_w4)       |                  |                  |
+------------------+------------------+------------------+------------------+
| depression\_     | Lifetime         | Binary           | -   0=no;        |
| l                | depression for   |                  | -   1=yes        |
| ifetime_proxy_s3 | proxy (c2011_w4) |                  |                  |
+------------------+------------------+------------------+------------------+
| d_12m_item_s3    | 12 month         | Binary           | -   0=no;        |
|                  | depression, one  |                  | -   1=yes        |
|                  | item (c2011_w4)  |                  |                  |
+------------------+------------------+------------------+------------------+
| d                | 12 month         | Binary           | -   0=no;        |
| epression_12m_s3 | depression from  |                  | -   1=yes        |
|                  | lifetime         |                  |                  |
|                  | diagnosis &      |                  |                  |
|                  | q4227/q4227a     |                  |                  |
|                  | items (c2011_w4) |                  |                  |
+------------------+------------------+------------------+------------------+
| d_lifeti         | Severity of      | Binary           | -   0=no;        |
| me_comparable_s3 | lifetime         |                  | -   1=yes        |
|                  | depressive       |                  |                  |
|                  | episode          |                  |                  |
|                  | comparable       |                  |                  |
|                  | (c2011_w4)       |                  |                  |
+------------------+------------------+------------------+------------------+
| d_1              | 12 month         | Binary           | -   0=no;        |
| 2m_comparable_s3 | depression       |                  | -   1=yes        |
|                  | comparable with  |                  |                  |
|                  | COVID substudy   |                  |                  |
|                  | (c2011_w4)       |                  |                  |
+------------------+------------------+------------------+------------------+
...
prevalence; variable `depression_lifetime_s3`) and in the previous 12 months
(point prevalence; variable `depression_12m_s3`). More precisely, the following
...
v)  At least two of the following three symptoms must be present: depressed mood
    (q4220a_s3:q4220d_s3), loss of interest (q4220e_s3, q4220f_s3) and decreased
    energy (q4222e_s3);

vi) An additional symptom or symptoms from the following ones should be present,
    to give a total of at least four: loss of confidence (q4222k_s3:q4222l_s3),
    excessive guilt (q4222n_s3), recurrent thoughts of death
    (q4222r_s3:q4222v_s3), diminished ability to concentrate
    (q4222h_s3:q4222j_s3), agitation or retardation (q4222f_s3, q4222fa_s3,
    q4222g_s3, q4222ga_s3), sleep disturbance (q4222c_s3, q4222d_s3), change in
    appetite (q4222a_s3, q4222b_s3) (please see criteria C1-C7).
...
versions of both lifetime (`d_lifetime_comparable_s3`) and recent
...
***#"VARIABLE `depression_lifetime_s3`: Depressive episode (F32)"
...
gen     d_lifetime_severity_s3 = .
replace d_lifetime_severity_s3 = 0 if (d_skipped==1 | d_crit_a==0 | d_crit_b==0 | d_lifetime_severity_total==0) & proxy==2
replace d_lifetime_severity_s3 = 1 if d_skipped==0 & d_crit_a==1 & d_crit_b==1      & d_lifetime_severity_total>=1 & d_lifetime_severity_total!=.
replace d_lifetime_severity_s3 = 2 if d_skipped==0 & d_crit_a==1 & d_crit_b==1      & d_lifetime_severity_total>=2 & d_lifetime_severity_total!=.
replace d_lifetime_severity_s3 = 3 if d_skipped==0 & d_crit_a==1 & d_screentotal==3 & d_lifetime_severity_total==3
label values   d_lifetime_severity_s3 severity
label variable d_lifetime_severity_s3 "Severity of lifetime depressive episode (c2011_w4)"
*#"NOTE: The criteria are applied incrementally, such that if a case
...
replace depression_lifetime_s3 = 0 if d_lifetime_severity_s3==0 & proxy==2
replace depression_lifetime_s3 = 1 if d_lifetime_severity_s3>=1 & d_lifetime_severity_s3 !=.
...
***#"VARIABLE `depression_12m_s3`: 12-month depression (combining `depression_lifetime_s3` + `d_12m_item_s3`)"
gen     depression_12m_s3 = .
replace depression_12m_s3 = 0 if depression_lifetime_s3==0 | d_12m_item_s3==0
replace depression_12m_s3 = 1 if depression_lifetime_s3==1 & d_12m_item_s3==1
label values   depression_12m_s3 yesno
label variable depression_12m_s3 "12 month depression from lifetime diagnosis + q4227/q4227a items (c2011_w4)"


***#"VARIABLE `d_12m_comparable_s3`: 12-month depression (comparable with COVID substudy) from lifetime diagnosis"

*#"Lifetime "comparable" (does not take into account exclusion criteria G2 and G3)"
gen     d_lifetime_comparable_s3 = .
replace d_lifetime_comparable_s3 = 0 if d_skipped==1 | d_crit_g1==0 | d_crit_b==0 | d_lifetime_severity_total==0
replace d_lifetime_comparable_s3 = 1 if d_skipped==0 & d_crit_g1==1 & d_crit_b==1 & d_lifetime_severity_total>=1 & d_lifetime_severity_total!=.
label values   d_lifetime_comparable_s3 severity
label variable d_lifetime_comparable_s3 "Severity of lifetime depressive episode comparable (c2011_w4)"

*#"12-month 'comparable', based on the intermediate 'lifetime comparable' and the individual item":
gen     d_12m_comparable_s3 = .
replace d_12m_comparable_s3 = 0 if d_lifetime_comparable_s3==0 | d_12m_item_s3==0
replace d_12m_comparable_s3 = 1 if d_lifetime_comparable_s3==1 & d_12m_item_s3==1
...
keep ID_ECS q0007a_result proxy d_lifetime_severity_s3 depression_lifetime_s3 depression_lifetime_proxy_s3 d_12m_item_s3 depression_12m_s3 d_lifetime_comparable_s3 d_12m_comparable_s3

codebook d_lifetime_severity_s3 depression_lifetime_s3 depression_lifetime_proxy_s3 d_12m_item_s3 depression_12m_s3 d_lifetime_comparable_s3 d_12m_comparable_s3
```
