...
|                | categorical score       |                | status (12-14) |
|                | (c2011_w4)              |                |                |
|                |                         |                | 1 = at risk of |
|                |                         |                | malnutrition   |
|                |                         |                | (8-11)         |
...
appetite, weight loss, and acute illness). The information on the three
remaining items was extracted from similar questions available in the interview
(mobility, neuropsychological problems, and BMI). These items were coded and the
resulting score was obtained following the MNA-SF's scoring instructions. The
total score was obtained by summing the answer given by the participant to each
of the 6 items. The higher the score, the better the participant's nutrition.
The six screening question was the following:

-   A (q3045_s3): Has food intake declined over the past 3 months due to loss of
    appetite, digestive problems, chewing or swallowing difficulties? (ranging
    from 0 severe decrease in food intake to 2 no decrease in food intake);

-   B (q3046_s3): Weight loss during the last 3 months (ranging from 0 weight
    loss greater than 3kg to 3 no weight loss);

-   C (q2041_movement_s3, q2046_going_s3): Mobility (ranging from 0 bed or chair
    bound to 2 goes out);

-   D (q3047_s3): Has suffered psychological stress or acute disease in the past
    3 months? (coding as 0 yes; 2 no);

-   E (q4233_s3, q4224_s3, q4225_causemed1_s3, q4225_causemed2_s3,
    q4225_causemed3_s3, q4225_causemed4_s3): Neuropsychological problems
    (ranging from 0 severe dementia or depression to 2 no psychological
    problems);

-   F (q2507_weight_s3, q2506_height_s3): Body Mass Index (ranging from 0 BMI
    less than 19 to 3 BMI 23 or greater).

    Note: To create this outcome a merge of two other outcomes is necessary. We
    will use the BMI variable (bmi_s3) from the dataset of "anthropometric
    measures" and the variable related to lifetime depression
    (lifetime_depression) from the dataset of "depression_ICD10".
...
merge 1:1 ID_ECS using "~\UAM\Marta Miret Garcia - Documentacion Edad con Salud\Edad con salud - Ola 4\Outcomes\Cohorte 2011\Outcome datasets\Outcome_depression_ICD10.dta", generate(newvar)
...
gen MNA_E = 2
replace MNA_E = 0 if depression_lifetime_s3==1
*# BMI
gen MNA_F = bmi_s3 
