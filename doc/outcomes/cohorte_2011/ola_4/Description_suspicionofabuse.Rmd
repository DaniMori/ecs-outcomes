...
asked in the course of an encounter between the elderly person and the
professional in the consultation room and are formulated by the professional
using simple language. The first question focuses on the assessment of the level
of dependency of the elderly person for some basic or instrumental activity of
daily living. The remaining questions refer to different types of abuse:
physical and sexual, psychological, financial, and neglect. The items are
dichotomous (0=No; 1=Yes), and are followed by items on its frequency on a
Likert scale. A positive response to any of the questions would indicate
suspicion of abuse. It is noteworthy that in our version we do not use the item
...
The questions refer to situations that occurred in the 12 months before the
completion of the questionnaire and only persons aged 65 and over were asked.

Variables indicating the presence and frequency of each type of abuse are
created, as well as a variable indicating the degree of dependency of the
elderly and a general variable about suspicion of abuse. These variables are
comparable with the previous waves. See 'explanation section' for more details.
...
-   Create a variable indicating the **degree of dependency** (`dependency_s3`),
    ...
    -   *Problematic dependence*: has received help, but had problems with
...
-   Create four variables, one for **each specific type of abuse**. For each
    ...
    indicates the presence or absence of abuse, and the second item, if abuse is
    present, its frequency on a Likert-type scale (1 occasional - 4 often).
...
`dependency_abuse_s3` is created. This is a dichotomous variable that includes:

-   **No abuse in dependency**: - Without help and no help needed - Help needed
    without any problems
-   **Abuse within dependency**: - Any positive score on the dichotomous items
    that indicate presence or absence on specific types of abuse. A intermediate
    variable 'abuse_types' is created to take them into account, which sums the
    answer of these dichotomous items.
...
keep ID_ECS q0007a_result_s3 proxy_s3 dependency_s3 abuse_negli_s3 abuse_psycho_s3 abuse_eco_s3 abuse_phys_s3 dependency_abuse_s3
codebook dependency_s3 abuse_negli_s3 abuse_psycho_s3 abuse_eco_s3 abuse_phys_s3 dependency_abuse_s3
```
