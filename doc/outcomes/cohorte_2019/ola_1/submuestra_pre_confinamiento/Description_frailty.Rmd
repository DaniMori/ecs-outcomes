---
title: "Edad con Salud -- cohort 2019 -- wave 1 -- 'Frailty'"
output:
  word_document:
...
+:===================+:=========================+:==============+:==============+
| frailty_index      | Frailty (c2019_w1)       | Categorical   | 0 non-frail;  |
|                    |                          |               |               |
|                    |                          |               | 1 frail       |
+--------------------+--------------------------+---------------+---------------+
| frailty_score      | Frailty score (c2019_w1) | Continuous    | 0 - 1         |
+--------------------+--------------------------+---------------+---------------+
...
```{stata load-data}
use "~\UAM\marta.miret@uam.es - Bases de datos maestras Edad con Salud\Ola_3\Cohorte_2019\Submuestra_1_preconfinamiento\Edad con Salud ola 3_cohorte 2019_base completa_Stata14.dta"
```
...
merge 1:1 ID_ECS using "~\UAM\marta.miret@uam.es - Documentacion Edad con Salud\Edad con salud - Ola 3\Outcomes\Cohorte 2019\Submuestra_1_preconfinamiento\Outcome datasets\Outcome_BMI.dta", keepusing(bmi) gen(merge_bmi)

recode bmi 0/2=0 3=1, gen(obesity)


** Chronic conditions

* TODO: Dataset update pending
merge 1:1 ID_ECS using "~\UAM\marta.miret@uam.es - Documentacion Edad con Salud\Edad con salud - Ola 3\Outcomes\Cohorte 2019\Submuestra_1_preconfinamiento\Outcome datasets\Chronic conditions.dta", keepusing(hypertension stroke oral_self lung diab_self angina asthma arthritis catar_self) gen(merge_chronic)

*** Directly merges in variables `hypertension`, `stroke`, `lung`,
*** `angina`, `asthma` and `arthritis`.

*** Oral impairment (Recode `oral_self` into a dichotomous variable)
recode oral_self 1/2=1, gen(oral_problems)

*** Diabetes (Recode `diab_self` into a dichotomous variable)
recode diab_self 1/2=1, gen(diabetes)

*** Cataracts (Recode `catar_self` into a dichotomous variable)
recode catar_self 1/2=1, gen(cataracts)


** Use of aid machines (Recode value `2` (= 'No') into `0`)
recode q2049_aids 2=0, gen(aid_machines)


** Grip Strength (weakness) (grip: 2 levels)

merge 1:1 ID_ECS using "~\UAM\marta.miret@uam.es - Documentacion Edad con Salud\Edad con salud - Ola 3\Outcomes\Cohorte 2019\Submuestra_1_preconfinamiento\Outcome datasets\Outcome_gripstrength_preconfinement.dta", keepusing(grip) gen(merge_grip)


** Vision difficulties (vision: 4 levels; note this does not match the
**   visual acuity measure used in Tyrovolas, 2018)

recode q2023_vis_diff q2024_vis_diff (888=.)
gen    vision = q2023_vis_diff + q2024_vis_diff
...
*   `q2016_sleep`.
* recode q2017_rested 1=0 2=0.25 3=0.5 4=0.75 5=1 888=.
...
merge 1:1 ID_ECS using "~\UAM\marta.miret@uam.es - Documentacion Edad con Salud\Edad con salud - Ola 3\Outcomes\Cohorte 2019\Submuestra_1_preconfinamiento\Outcome datasets\Outcome_physicalactivity.dta", keepusing(physical) gen(merge_physical)

generate physical_week = physical / 2
...
label variable frailty_index "Frailty (c2019_w1)"
...
save "~\UAM\marta.miret@uam.es - Documentacion Edad con Salud\Edad con salud - Ola 3\Outcomes\Cohorte 2019\Submuestra_1_preconfinamiento\Outcome datasets\Outcome_frailty.dta", replace
```
