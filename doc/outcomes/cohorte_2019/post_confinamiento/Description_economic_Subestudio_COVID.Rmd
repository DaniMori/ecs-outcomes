...
+===============+=====================================================================+=============+=====================+
| noise         | Noise at home (c2019_wCOVID)                                        | Dichotomous | 0 = Yes             |
|               |                                                                     |             |                     |
|               |                                                                     |             | 1 = No              |
+---------------+---------------------------------------------------------------------+-------------+---------------------+
...
-   **noise**: Self-reported response of whether there is a lot of noise in the house and it does not let them rest.
...
-   **noise**: Reversed item `ECON1_1` recoded as `0` (*Yes*) or `1` (*No*).
...
gen noise = ECON1_1 //Reversed item
replace noise = 0 if noise == 1
replace noise = 1 if noise == 2
replace noise = . if ECON1_1 == 9
...
label variable noise "Noise at home (c2019_wCOVID)"
label values noise BOOL_LABELS
...
codebook noise internet home_score economy unemployment
```

```{stata save-data}
keep ID_ECS ESTADO_ENTREVISTA noise internet home_score economy unemployment
