...
Hay 28 casos "0" que originalmente se codifican como "1";
5 de ellos corresponden a respuestas "DK" o "Refuse" en `q4224a`
(ver segunda tabla).
Otros 23 casos corresponden a casos en los que `q4224==1`;
cuando se da esta condición, tanto `q4224a` como `q4224b` son "missing"
(y por tanto distintos de "1") por lo que
se cumple siempre la condición del primer criterio de reemplazo,
haciendo que los valores de las causas médicas sean irrelevantes.
...
Debido a las diferencias entre los resultados en G3,
hay diferencias en 21 casos que originalmente son codificados como "1" y
en la revisión como "0".
...
La diferencia se debe a las dos últimas condiciones del algoritmo original
(` | (q4220a!=1 & q4220b!=1 & q4220c!=1 & q4220d!=1 & q4220e!=1 & q4220f!=1) | serie_symp<2`).
De acuerdo a mi criterio, no deberían utilizarse para considerar si se ha
saltado o no el módulo de depresión, ya que esos ítems están dentro del módulo
y, aparte de utilizar información muy compleja que no deja claro por qué esos
criterios se consdieran como saltos del módulo de depresión.
Por otro lado, esta versión revisada da 91 "missing", que corresponden con los
casos "proxy", lo cual pone de relieve también que el cálculo se ha hecho
correctamente con los datos presentes en la BDD.
...
Se calcula primero un criterio inicial de severidad considerando si la
...
2510 valores "missing" son "0" en el algoritmo original, y hay otros 17 casos
que tienen valores discrepantes de severidad
(en los cuales también cambia el diagnóstico)
...
y si el módulo de depresión se ha saltado o no
(en este caso, la variable `d_screening` no hace falta puesto que los resultados
al añadirla no cambian, dado que ya se tienen en cuenta los casos negativos y
positivos en la variable `d_skipped`).
...
17 respuestas "0" son "1" en el algoritmo original.
Las discrepancias se deben, principalmente,
