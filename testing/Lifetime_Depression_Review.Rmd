...
corresponden a respuestas "DK" o "Refuse" en `q4224a`.
Los 23 casos qeu son "1" en el algoritmo original y "0" en el revisado
...
* # Original:
gen d_crit_a_org = .
replace d_crit_a_org = 0 if d_crit_g1!=1 | d_crit_g2!=1 | d_crit_g3!=1
replace d_crit_a_org = 1 if d_crit_g1==1 & d_crit_g2==1 & d_crit_g3==1
...
Curiosamente, a pesar de las diferencias entre los resultados en G3,
no hay diferencias en los casos que tienen "0" o "1" originalmente.
No obstante, este resultado puede ser (probablemente)
dependiente de esta muestra concreta;
es decir, no hay que asumir que los dos algoritmos son necesariamente
equivalentes (para los valores no perdidos).
...
ítems implica un síntoma negativo (en principio).
...
tab d_crit_b1_org d_crit_b1, missing
```
...
tab d_crit_b2_org d_crit_b2, missing
```
...
tab d_crit_b3_org d_crit_b3, missing
```
...
Cuando la respuesta a `q4222l` es _negativa_, el ítem `q4222k` se salta,
...
* # 1 MISSING WAS ORIGINALLY a 0, 2730 were originally 1's in `d_symtotal`
```
...
* # NOTE: This variable is NEVER used in this version of the
* # "lifetime depression" algorithm (but is used in other(s)). Instead, the
* # variable `d_screening` (see lines 351-360) is used in the lifetime
* # depression diagnostic (variable `depression_lifetime`, lines 362-368)
```
...
[Diagnóstico final de depresión](#diagnostico-final-de-depresion).)
...
o mayor que 3, o menor que 3. Esto ocurrirá si la suma total de síntomas
positivos y "missing" es igual o mayor que 3 (y no se cumple ningún otro
...
* # 2527 MISSING WERE ORIGINALLY 0's
```

2527 valores "missing" son "0" en el algoritmo original.
...
* # 96 MISSING WERE ORIGINALLY 0's, 1 MISSING WAS ORIGINALLY A 1, AND 3 "1"'s
* # WERE "0"'s
```

96 respuestas "missing" es originalmente "0" y 1 es "1", y 3 "1" son "0" en el
algoritmo orginal.

# Sintaxis completa (Stata 13)

```{stata review, eval=FALSE}
*Previous recoding of the symptomatology questions
recode q4220a-q4222v (1=1) (2=0) (888 999=.) 
...
* _non-physical_ and thus the criterion is fulfilled (for simplicity, as this
* contemplates negative AND missing responses), and then the cases where
* physical causes are attributed are discarded.
gen d_crit_g3 = .
replace d_crit_g3 = 1 if (q4224a==2 | q4224b==2) | q4224==1
replace d_crit_g3 = 0 if (q4224a==1 & q4224b==1) | (q4224==1 & (q4225_causemed12==1 | q4225_causemed23==1 | q4225_causemed24==1 | q4225_causemed25==1 | q4225_causemed30==1 | q4225_causemed31==1))
label variable d_crit_g3 "G3 criterion for depression" 
* # Contingency between "original" (columns) and newly computed values:
* #
* #         G3 |
* #  criterion |
* #        for |          d_crit_g3_org
...
* #          . |         0          5      2,730 |     2,735 
* # -----------+---------------------------------+----------
* #      Total |         6        266      2,730 |     3,002 
* # 
* # 5 cases (4 DK, 1 Refuse) in q4224a go into "1" in the original version and
* # to "missing" in this version
* # The 23 cases are due to the fact that both q4224a and q4224b are "missing"
* # when q4224 is "1", and thus they are different from "1" and the first
* # replacement criterion (in the original algorithm) is always true.
...
* known NOT to be met, as the value of the other two don´t matter.
gen d_crit_a = .
replace d_crit_a = 0 if d_crit_g1==0 | d_crit_g2==0 | d_crit_g3==0
replace d_crit_a = 1 if d_crit_g1==1 & d_crit_g2==1 & d_crit_g3==1
label variable d_crit_a "A criterion for depression"
* # MISSING WERE ORIGINALLY 0's (in spite of the differences among the results
* # in B3, there are no differences in the cases with "0"´s or "1"´s, curiously,
* # although this result is probably sample-dependent)
...
* # 2 MISSING WERE ORIGINALLY 0's, 2730 were originally 1's
...
* that have been prompted with at least one of these items. If they haven´t,
