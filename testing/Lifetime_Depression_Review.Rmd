---
title: "Revisión de la variable \"Lifetime Depression (CIE-10)\" de Chorte 2019, Ola
  1"
...
```{stata initial-recoding, results='hide'}
...
Hay 2528 valores "missing" que se codifican como "0" y 54 como "1"
...
(Nótese que aquí no coinciden los 32 `0` generados con los 10
que hay finalmente, pero se debe a que 22 valores inicialmente de `0`
se recodifican, tal y como se indica arriba.)
...
Presumiblemente, se debe a que hay un error en la segunda instrucción
`replace`, que debería ser: `replace d_c3_org = **.** ...`
en lugar de `replace d_c3_org = **1** ...`.
...
Presumiblemente, estos "missing" son los "1" que deberían ser "missing" en C3.
...
Además, el módulo completo de depresión se puede saltar si se da alguna de 
estas condiciones:

* Alguno de los siguientes ítems tiene una respuesta
  que sea distinta de _positiva_: `q4202`, `q4211`, `q4213`

* Se cumple la condición de salto del filtro F4217
  (lo cual implica `q4214==1` Y `q4215==4` Y `q4216==4`).
  
* Se cumple la condición de salto del filtro F4221 (lo cual implica
  que no haya ninguna respuesta "Sí" (=1) en los ítems `q4220*`).

* Se cumple la condición de salto del filtro F4223 (lo cual implica
  que haya menos de 2 respuestas "Sí" (=1) en los ítems `q4220*` y `q4222*`).

**NOTA:** Las variables de los filtros NO se almacenan en la BDD, y por lo
tanto hay que calcularlas para poder aplicarlas.
En el caso de F4217, se asigna el valor de verdad de este filtro
sólo cuando `q4214` no es "missing", ya que ello implica que
`q4215` y `q4216` tampoco lo son.

(**NOTA**: Se ha considera que el modulo ha sido "saltado" por los participantes
a los que se ha preguntado por _al menos_ alguno de estos ítems.
...
replace d_skipped_org = 1 if q4202_alt==0 | q4211==0 | q4213==0 | (q4214==1 & q4215==4 & q4216==4) | q4218==0 | (q4220a!=1 & q4220b!=1 & q4220c!=1 & q4220d!=1 & q4220e!=1 & q4220f!=1) | serie_symp_org<2

* # Revision:

* ## Generate filter variables first:
...
91 "missing" se codifican como "1" en el algoritmo original.
Se debe presumiblemente a los casos "proxy", ya que originalmente no se tiene
en cuenta si dan lugar o no a "missing".

(**NOTA**: En la versión revisada no se usa nunca esta variable para
la severidad de "lifetime depression". En su lugar, se usa la variable
`d_screening`, ver apartado
[Diagnóstico final de depresión](#Diagnóstico_final_de_depresión).)

### Severidad

En esta versión, el código primero asigna "0" a los participantes que dieron
tres respuestas negativas al test de screening inicial (ítems `q4200`-`q4202`).
...
Además, incluimos la posibilidad de que haya demasiados "missing" como
para no poder calcular si la suma total de síntomas (criterios B + C) es
_igual o mayor que 4_ o _menor que 4_.
Esto ocurrirá si la suma total de síntomas positivos y "missing"
es igual o mayor que 4 (y no se cumple ningún otro criterio).
Por tanto, se calcula el número de "missing"s, y después se obtiene
...
replace d_lifetime_severity = 0 if d_screening==0
replace d_lifetime_severity = 0 if d_screening==1 & d_crit_a==0 | d_crit_b==0      | d_lifetime_severity_total==0
replace d_lifetime_severity = 1 if d_screening==1 & d_crit_a==1 & d_crit_b==1      & d_lifetime_severity_total>=1 & d_lifetime_severity_total!=.
replace d_lifetime_severity = 2 if d_screening==1 & d_crit_a==1 & d_crit_b==1      & d_lifetime_severity_total>=2 & d_lifetime_severity_total!=.
replace d_lifetime_severity = 3 if d_screening==1 & d_crit_a==1 & d_screentotal==3 & d_lifetime_severity_total==3

* # ALTERNATIVE: Use `d_skipped` to determine whether the value of
* # `d_lifetime_severity` is valid or not, and then fill in with the ones that
* # have a value of "0" in the screening (this turns 90 "missing" to "0").
* replace d_lifetime_severity = 0 if d_skipped==1
* replace d_lifetime_severity = 0 if d_skipped==0 & d_crit_a==0 | d_crit_b==0      | d_lifetime_severity_total==0
...
229 valores "missing" tienen diferentes valores de severidad
en el algoritmo original, habiendo 34 que tienen al menos valor "1".
Además, hay 14 casos con valores distintos de "0" en la versión original que
...
El porcentaje de casos con depresión que se obtiene con el algoritmo revisado
es menor (1,81%) que con el algoritmo original.
El total de casos con valores válidos es menor que el obtenido originalmente,
en parte debido al número de valores perdidos en el número total de síntomas.

# Versión "liberal (específica)" en la asignación del diagnóstico

Esta versión es menos conservadora a la hora de asignar un diagnóstico de
depresión cuando hay falta de información (respuestas "DK" o "Refuse").
Concretamente, en los casos en los que se han dado respuestas no válidas,
se parte del supuesto de que el criterio diagnóstico en cuestión **NO**
se cumple.
De esta manera, se tiende únicamente a diagnosticar depresión cuando hay
información suficiente como para determinar que se da el diagnóstico
(i.e., maximiza la _especificidad_).
...
- Tener en cuenta los filtros en el cuestionario como respuestas _negativas_;
  es decir, asumir que si un ítem ha sido saltado,
  considerar de que su "valor diagnóstico" va a ser negativo.

- Por consistencia, generar siempre las variables nuevas como "missing"
  en primer lugar, y después calcular los valores no perdidos
  (esto se utiliza para los síntomas; para los criterios diagnósticos no ya que
  es más sencillo generar primero los "missing" y después rellenar con los
  valores válidos).
...
```{stata load-data-2, include=FALSE}
* Cohorte 2019, linea base:
use "~\UAM\marta.miret@uam.es - Bases de datos maestras Edad con Salud\Ola_3\Cohorte_2019\rawdata_c2019w1.dta", clear
```

En el algoritmo original, las preguntas de sintomatología se recodifican
como 0/1 (o "missing").
Dado que los "DK"/"Refuse" se deben considerar como "ausencia de síntoma" es
necesario tratarlos como "0".
Dado que el tratamiento es diferente entre el algoritmo original y el revisado,
creo nuevas variables con el sufijo `_alt` para el algoritmo original,
y así evitar sobreescribir sus valores.

```{stata initial-recoding-2, results='hide'}
gen q4220a_alt = q4220a
gen q4220b_alt = q4220b
gen q4220c_alt = q4220c
gen q4220d_alt = q4220d
gen q4220e_alt = q4220e
gen q4220f_alt = q4220f
gen q4222a_alt = q4222a
gen q4222b_alt = q4222b
gen q4222c_alt = q4222c
gen q4222d_alt = q4222d
gen q4222e_alt = q4222e
gen q4222f_alt = q4222f
gen q4222fa_alt = q4222fa
gen q4222g_alt = q4222g
gen q4222ga_alt = q4222ga
gen q4222h_alt = q4222h
gen q4222i_alt = q4222i
gen q4222j_alt = q4222j
gen q4222k_alt = q4222k
gen q4222l_alt = q4222l
gen q4222m_alt = q4222m
gen q4222n_alt = q4222n
gen q4222o_alt = q4222o
gen q4222p_alt = q4222p
gen q4222q_alt = q4222q
gen q4222r_alt = q4222r
gen q4222s_alt = q4222s
gen q4222t_alt = q4222t
gen q4222u_alt = q4222u
gen q4222v_alt = q4222v

*Previous recoding of the symptomatology questions
recode q4220a_alt-q4222v_alt (1=1) (2=0) (888 999=.)
```
...
El criterio es _negativo_ cuando tanto `q4211` como `q4213` son distintos a "1".
**SIN EMBARGO**, la entrevista salta uno de los dos ítems o el otro, de forma
que **SIEMPRE** se va a dar el caso de uno de ellos va a ser "missing".
Por lo tanto, el criterio es _positivo_ cuando uno de los dos ítems es "1",
"missing" cuando ambos criterios son "missing", y _negativo_ en otro caso.

```{stata criterion-g1-2}
*G1 criterion: "The depressive episode should last for at least 2 weeks"
...
[criterio G1](#criterio-g1);
la única diferencia es que si `q4225_causemed2` no se ha seleccionado, su valor
puede ser "0" o "missing", por lo que solamente se tiene en cuenta `q4233`
para determinar si el valor de este criterio es "missing".

```{stata criterion-g2-2}
*G2 criterion: "There have been no hypomanic or manic symptoms sufficient to meet the criteria for hypomanic or manic episode at any time in the individual's life"
...
* # Revision:
gen d_crit_g2 = q4233!=1 & q4225_causemed2!=1
...
Todos los "missing" se codifican como "1" en el algoritmo original.
Esto sería correcto ya que G2 es un criterio de exclusión.
Por lo tanto, si no tenemos información suficiente para determinar que se da
el criterio de exclusión, asumimos que NO se cumple
(es decir, NO se da un episodio maníaco o hipomaníaco,
y por lo tanto sí se da la condición para poder diagnosticar depresión).
...
a causas físicas, i.e. `q4224a` es "No", o bien los episodios **NO SIEMPRE**
fueron atribuibles a cusas físicas, i.e. `q4224b` es "No").
Cuando se consultó a un médico, se considera el caso de que se dio _ALGUNA_
las causas físicas (en cuyo caso no se cumple el criterio),
o bien no se dió ninguna.
En este último caso, es necesario considerar que la "causa médica" es
"distinta de 1", ya que se dan casos en los que está codificada como "0" y otros
en los que está codificada como "missing".
Además, si se consultó con un médico, los valores tanto de `q4224a` como de
`q4224b` son ambos "missing", por lo que el único caso para considerar este
criterio como "missing" es cuando estos tres ítems son missing.

```{stata criterion-g3-2}
*G3 criterion: "The episode is not attributable to psychoactive substance use or to any organic mental disorder"
...
* # Revision:
gen d_crit_g3 = (q4224a!=1 & q4224a!=.) | (q4224b!=1 & q4224b!=.) | (q4224==1 & (q4225_causemed12!=1 & q4225_causemed23!=1 & q4225_causemed24!=1 & q4225_causemed25!=1 & q4225_causemed30!=1 & q4225_causemed31!=1))
replace d_crit_g3 = . if q4224==. & q4224a==. & q4224b==.

tab d_crit_g3_org d_crit_g3, missing
```

Hay 23 casos "0" que originalmente se codifican como "1",
que corresponden a casos en los que `q4224==1`;
cuando se da esta condición, tanto `q4224a` como `q4224b` son "missing"
(y por tanto distintos de "1") por lo que en el algoritmo original
se cumple siempre la condición del primer criterio de reemplazo,
haciendo que los valores de las causas médicas sean irrelevantes.
Esto da lugar a casos diagnosticados como "1" cuando su diagnóstico debería
depender de las respuestas de los ítems `q225_*`.
...
Cuando **alguno** de ellos no se cumple (== "0"), entonces el criterio general
**NO** se cumple (i.e. los otros dos no importan).
Generamos por tanto todos los valores primero como "missing",
y después asignamos los valores de verdad en función de estos criterios.
...
Los valores "missing" se codifican como "0" en el algoritmo original.
Debido a las diferencias entre los resultados en G3,
hay diferencias en 16 casos que originalmente son codificados como "1" y
en la revisión como "0".
...
La explicación para el cálculo es la siguiente: Cuando **ALGUNA** de
las respuestas es _positiva_, el sínoma es positivo.
Cuando hay alguna respuesta "missing" en `q4220b` o `q4220d` se puede deber a
algún salto debido al filtro en `q4220a` o `q4220c`, respectivamente.
Por lo tanto, para tener en cuenta si la respuesta es "missing" solamente hay
que considerar si `q4220a` Y `q4220c` son "missing".
Pero dado que si hay alguna respuesta en `q4220a` tiene que haberla
necesariamente en `q4220c` también,
basta con comprobar si `q4220a` es "missing".

```{stata criterion-b1-2}
*B criterion: "At least two of the following three symptoms must be present"
...
* # Original:
gen d_b1_org = 0
replace d_b1_org = 1 if q4220a_alt==1 | q4220b_alt==1 | q4220c_alt==1 | q4220d_alt==1
replace d_b1_org = . if q4220a_alt==. & q4220b_alt==. & q4220c_alt==. & q4220d_alt==.

* # Revision:
gen d_b1 = q4220a==1 | q4220b==1 | q4220c==1 | q4220d==1
replace d_b1 = . if q4220a==.
...
La explicación es similar a la del criterio B1
(salvo porque en este caso no hay filtros,
por lo que no importa si `q4220f` es o no "missing").

```{stata criterion-b2-2}
*B2: "Loss of interest or pleasure in activities that are normally pleasurable"

* # Original:
gen d_b2_org = 0
replace d_b2_org = 1 if q4220e_alt==1 | q4220f_alt==1
replace d_b2_org = . if q4220e_alt==. & q4220f_alt==.

* # Revision:
gen d_b2 = q4220e==1 | q4220f==1
replace d_b2 = . if q4220e==.
...
```{stata criterion-b3-2}
*B3: "Decreased energy or increased fatigability"

* # Original:
gen d_b3_org = 0
replace d_b3_org = 1 if q4222e_alt==1
replace d_b3_org = . if q4222e_alt==.

* # Revision:
gen d_b3 = q4222e==1 if q4222e!=.

tab d_b3_org d_b3, missing
```

Tres casos "0" se codifican como "missing" en el algoritmo original,
debido a la recodificación inicial de respuestas no válidas como "missing".
...
La suma de síntomas debe ser inferior a 2 para determinar que
NO se cumple el criterio B.
En teoría habría que tener en cuenta si hay demasiados valores "missing"
como para poder determinar si hay más o menos de 2 síntomas,
pero se parte del supuesto de que los "missing" son negativos
(salvo que **TODOS** los valores sean "missing",
en cuyo caso se asume que el criterio está indefinido y se le asigna "missing").
...
gen d_crit_b = d_screentotal >= 2 if d_screentotal != .
...
En ambos casos, tanto en la variable auxiliar como en el criterio B, los
"missing" siempre se codifican como "0" en el algoritmo original.
Se debe a que cuando todos los valores son "missing" el número de síntomas
que se cuenta en total es "0" en el algoritmo original.
...
* # Original:
gen d_c1_org = 0
replace d_c1_org = 1 if q4222k_alt==1 | q4222l_alt==1 | q4222m_alt==1
replace d_c1_org = . if q4222k_alt==. & q4222l_alt==. & q4222m_alt==.

* # Revision:
gen d_c1 = q4222k==1 | q4222l==1 | q4222m==1
replace d_c1 = . if q4222k==. & q4222l==.

tab d_c1_org d_c1, missing
```

1 "0" es "missing" en el algoritmo original,
debido a la recodificación inicial de respuestas no válidas como “missing”.

#### Criterio C2

```{stata criterion-c2-2}
*C2: "Unreasonable feelings of self-reproach or excessive and inappropriate guilt"

* # Original:
gen d_c2_org = 0
replace d_c2_org = 1 if q4222n_alt==1
replace d_c2_org = . if q4222n_alt==.

* # Revision:
gen d_c2 = q4222n==1 if q4222n!=.

tab d_c2_org d_c2, missing
```

Tres "0" son "missing" en el algoritmo original,
debido a la recodificación inicial de respuestas no válidas como “missing”.
...
Cuando **ALGUNA** de las respuestas es _positiva_ el síntoma es _positivo_.
Cuando `q4222t` es _negativa_, los dos últimos ítems se saltan por lo que sólo
las tres primeras respuestas se deben tener en cuenta para asignar un "missing".
Pero dado que si hay alguna respuesta en `q4222r` tiene que haberla
necesariamente en `q4222s` y `q4222t`,
basta con comprobar si `q4222r` es "missing".

(**NOTA**: ¿Es posible que convenga considerar que _SÓLO_ `q4222r`,
y no `q4222s`, indica pensamientos de muerte **recurrentes**?)

```{stata criterion-c3-2}
*C3: "Recurrent thoughts of death or suicide, or any suicidal behaviour"

* # Original:
gen d_c3_org = 0
replace d_c3_org = 1 if q4222r_alt==1 | q4222s_alt==1 | q4222t_alt==1 | q4222u_alt==1 | q4222v_alt==1
replace d_c3_org = 1 if q4222r_alt==. & q4222s_alt==. & q4222t_alt==. & q4222u_alt==. & q4222v_alt==.

* # Revision:
gen d_c3 = q4222r==1 | q4222s==1 | q4222t==1 | q4222u==1 | q4222v==1
replace d_c3 = . if q4222r==.

tab d_c3_org d_c3, missing
```

Los valores "missing" son "1" en el algoritmo original.
Presumiblemente, se debe a que hay un error en la segunda instrucción
`replace`, que debería ser: `replace d_c3_org = **.** ...`
en lugar de `replace d_c3_org = **1** ...`.
...
La explicación es similar a la del criterio C3.

```{stata criterion-c4-2}
*C4: "Complaints or evidence of diminished ability to think or concentrate, such as indecisiveness or vacillation"

* # Original:
gen d_c4_org = 0
replace d_c4_org = 1 if q4222h_alt==1 | q4222i_alt==1 | q4222j_alt==1
replace d_c4_org = . if q4222h_alt==. & q4222i_alt==. & q4222j_alt==.

* # Revision:
gen d_c4 = q4222h==1 | q4222i==1 | q4222j==1
replace d_c4 = . if q4222h==.

tab d_c4_org d_c4, missing
```

Un "0" es "missing" en el algoritmo original,
debido a la recodificación inicial de respuestas no válidas como “missing”.
...
```{stata criterion-c5-2}
*C5: "Change in psychomotor activity, with agitation or retardation (either subjective or objective)"

* # Original:
gen d_c5_org = 0
replace d_c5_org = 1 if (q4222f_alt==1 | q4222fa_alt==1) | (q4222g_alt==1 | q4222ga_alt==1)
replace d_c5_org = . if q4222f_alt==. & q4222fa_alt==. & q4222g_alt==. & q4222ga_alt==.

* # Revision:
gen d_c5 = (q4222f==1 | q4222fa==1) | (q4222g==1 | q4222ga==1)
replace d_c5 = . if q4222f==.
...
```{stata criterion-c6-2}
*C6: "Sleep disturbance of any type"

* # Original:
gen d_c6_org = 0
replace d_c6_org = 1 if q4222c_alt==1 | q4222d_alt==1
replace d_c6_org = . if q4222c_alt==. & q4222d_alt==.

* # Revision:
gen d_c6 = q4222c==1 | q4222d==1
replace d_c6 = . if q4222c==.
...
```{stata criterion-c7-2}
*C7: "Change in appetite (decrease or increase) with corresponding weight change"

* # Original:
gen d_c7_org = 0
replace d_c7_org = 1 if q4222a_alt==1 & q4222b_alt==1
replace d_c7_org = . if q4222a_alt==. & q4222b_alt==.

* # Revision:
gen d_c7 = q4222a==1 & q4222b==1
replace d_c7 = . if q4222a==.
...
2730 "missing" son "1" en el algoritmo original.
Cuando los "missing" se codifican como "0" en el algoritmo original,
o viceversa, no hay ningún problema, ya que el comando `rowtotal` los ingora
para calcular la suma total por lo que da lo mismo si valen "0" o son "missing".
El problema está en C3, donde los "missing" deberían ser o bien "missing" o "0".
Esto hace que se esté contabilizando un síntoma de más para aquellos casos que
no tienen datos en alguno de los ítems utilizados para calcular el criterio C3.

### Salto del módulo de depresión al completo

Este criterio está definido originalmente como
_todas las respuestas en el módulo de 'lifetime depression' son "missing"_.
Sin embargo, este criterio no es estrictamente confiable, dado que
_no es imposible_ que alguien que **NO** haya saltado el módulo decida omitir
("DK" o "Refuse") _todas las respuestas_.
El único punto de entrada al módulo de 'lifetime depression' es el ítem `q4218`
...
* # Original:
egen serie_symp_org = rowtotal (q4220a_alt-q4222v_alt)
gen d_skipped_org = 0
recode q4202 q4211 q4213 q4218 (1=1) (.=.) (else=0)
replace d_skipped_org = 1 if q4202==0 | q4211==0 | q4213==0 | (q4214==1 & q4215==4 & q4216==4) | q4218==0 | (q4220a_alt!=1 & q4220b_alt!=1 & q4220c_alt!=1 & q4220d_alt!=1 & q4220e_alt!=1 & q4220f_alt!=1) | serie_symp_org<2

* # Revision:
* # Aquí faltaría la recodificación de los ítems q4220a-q4222v en 0/1;
* #   se utilizan en su lugar las versiones `_alt` ya creadas anteriormente,
* #   pero ha de aplicarse en este punto en la versión completa del algoritmo.

* ## Generate filter variables first:
gen  f4217     = q4214==1 & q4215==4 & q4216==4 if q4214!=.
egen f4221_sum = rowtotal (q4220a_alt-q4220f_alt), missing
egen f4223_sum = rowtotal (q4220a_alt-q4222v_alt), missing
...
91 valores "mising" y 7 "0" se codifican como "1" en el algoritmo original.
Se debe presumiblemente a los casos "proxy", ya que originalmente no se tiene
en cuenta si dan lugar o no a "missing".

### Severidad

En esta versión, el código asigna "0" a los participantes que saltaron el
módulo de depresió al completo (`d_skipped`).
Después se calcula un criterio inicial de severidad considerando si la
suma total de síntomas es menor que 4, igual o mayor que 4, 6 u 8.
Esta severidad se obtiene en orden creciente,
de forma que cada nivel de severidad sobreescribe el anterior.  
...
(**NOTA:** En este caso, la variable `d_screening` no hace falta,
al contrario que en la versión "conservadora",
puesto que los resultados al añadirla no cambian.
Esto se debe a que los casos que no entran en el módulo de depresión se
consideran negativos y ya se han tenido en cuenta en la variable `d_skipped`.)
...
* # Original:
gen d_lifetime_severity_org = .
replace d_lifetime_severity_org = 0 if d_crit_a_org==0 | d_crit_b==0 | (d_screentotal_org==2 & d_symtotal_org<2) | (d_screentotal_org==3 & d_symtotal_org==0)
replace d_lifetime_severity_org = 1 if d_crit_a_org==1 & ((d_screentotal_org==2 & d_symtotal_org>=2) | (d_screentotal==3 & d_symtotal_org>=1))
replace d_lifetime_severity_org = 2 if d_crit_a_org==1 & ((d_screentotal_org==2 & d_symtotal_org>=4) | (d_screentotal==3 & d_symtotal_org>=3))
replace d_lifetime_severity_org = 3 if d_crit_a_org==1 & (d_screentotal_org==3 & d_symtotal_org>=5)
...
91 valores "missing" son "0" en el algoritmo original,
debido a los casos "proxy". Hay otros 14 casos que tienen
valores discrepantes de severidad (en los cuales también cambia el diagnóstico).

### Diagnóstico final de depresión

El cálculo es similar al algoritmo original, pero se tiene en cuenta solamente
el valor de severidad de depresión calculado anteriormente,
ya que se ha considerado ya si el módulo de depresión se ha saltado o no
en el cálculo de la severidad.

```{stata liferime-depression-2}
* # Original:
gen depression_lifetime_org = .
replace depression_lifetime_org = 1 if d_lifetime_severity_org >=1 
replace depression_lifetime_org = 0 if proxy==2 & (d_lifetime_severity_org ==0 | d_skipped_org==1)
...
14 respuestas "0" son "1" en el algoritmo original.
Las discrepancias se deben, principalmente,
a los criterios G3 y C3.
Sería necesario modificar G3 para tener en cuenta los valores perdidos en
`q4224a` y `q4224b` cuando no se alcanzan esos ítems,
y codificar los "missing" en C3 como "missing" o "0".

## Sintaxis completa (Stata 13)

```{stata reload-data-2, include=FALSE}
use "~\UAM\marta.miret@uam.es - Bases de datos maestras Edad con Salud\Ola_3\Cohorte_2019\rawdata_c2019w1.dta", clear
```

### Revisión

```{stata review-2, results='hide'}
***Depressive episode (F32)
...
gen d_crit_g2_org = 1
replace d_crit_g2_org = 0 if q4233==1 | q4225_causemed2==1 
label variable d_crit_g2_org "G2 criterion for depression"

*G3 criterion: "The episode is not attributable to psychoactive substance use or to any organic mental disorder"
gen d_crit_g3_org = 0
replace d_crit_g3_org = 1 if (q4224a!=1 | q4224b!=1) | (q4224==1 & q4225_causemed12!=1 & q4225_causemed23!=1 & q4225_causemed24!=1 & q4225_causemed25!=1 & q4225_causemed30!=1 & q4225_causemed31!=1)
replace d_crit_g3_org = . if q4224a==. & q4224b==. & q4224==. & q4225_causemed12==. & q4225_causemed23==. & q4225_causemed24==. & q4225_causemed25==. & q4225_causemed30==. & q4225_causemed31==.
label variable d_crit_g3_org "G3 criterion for depression" 
...
gen d_skipped_org = 0
recode q4202 q4211 q4213 q4218 (1=1) (.=.) (else=0)
replace d_skipped_org = 1 if q4202==0 | q4211==0 | q4213==0 | (q4214==1 & q4215==4 & q4216==4) | q4218==0 | (q4220a!=1 & q4220b!=1 & q4220c!=1 & q4220d!=1 & q4220e!=1 & q4220f!=1) | serie_symp_org<2
...
gen d_lifetime_severity_org = .
replace d_lifetime_severity_org = 0 if d_crit_a_org==0 | d_crit_b_org==0 | (d_screentotal_org==2 & d_symtotal_org<2) | (d_screentotal_org==3 & d_symtotal_org==0)
replace d_lifetime_severity_org = 1 if d_crit_a_org==1 & ((d_screentotal_org==2 & d_symtotal_org>=2) | (d_screentotal_org==3 & d_symtotal_org>=1))
replace d_lifetime_severity_org = 2 if d_crit_a_org==1 & ((d_screentotal_org==2 & d_symtotal_org>=4) | (d_screentotal_org==3 & d_symtotal_org>=3))
replace d_lifetime_severity_org = 3 if d_crit_a_org==1 & (d_screentotal_org==3 & d_symtotal_org>=5)
label variable d_lifetime_severity_org "Severity of lifetime depressive episode (c2019_w1)"
*label define severity 0 "No" 1 "Mild" 2 "Moderate" 3 "Severe"
label values d_lifetime_severity_org severity
...
*label define yesno 0 "No" 1 "Yes"
label values depression_lifetime_org yesno
label variable depression_lifetime_org "Lifetime depression (c2019_w1)"
```
...
El porcentaje de casos con depresión que se obtiene con el algoritmo revisado
es ligeramente (0,58%) menor que con el algoritmo original.
El total de casos con valores válidos es menor que el obtenido originalmente,
en parte debido al número de valores perdidos en el número total de síntomas.

# Comparación de las dos versiones del algoritmo

## Sintaxis desglosada (Stata 13)

```{stata reload-data-3, include=FALSE}
use "~\UAM\marta.miret@uam.es - Bases de datos maestras Edad con Salud\Ola_3\Cohorte_2019\rawdata_c2019w1.dta", clear
```

```{stata initial-recoding-3, results='hide'}
gen q4220a_alt = q4220a
gen q4220b_alt = q4220b
gen q4220c_alt = q4220c
gen q4220d_alt = q4220d
...
```{stata criterion-g1-3}
*G1 criterion: "The depressive episode should last for at least 2 weeks"

* # Revision 1:
gen d_crit_g1_rev1 = .
replace d_crit_g1_rev1 = 0 if q4211==2 | q4213==2
replace d_crit_g1_rev1 = 1 if q4211==1 | q4213==1

* # Revision 2:
gen d_crit_g1 = q4211==1 | q4213==1
replace d_crit_g1 = . if q4211==. & q4213==.

tab d_crit_g1_rev1 d_crit_g1, missing
```

#### Criterio G2

```{stata criterion-g2-3}
*G2 criterion: "There have been no hypomanic or manic symptoms sufficient to meet the criteria for hypomanic or manic episode at any time in the individual's life"

* # Revision 1:
gen d_crit_g2_rev1 = .
replace d_crit_g2_rev1 = 0 if q4233==1 | q4225_causemed2==1
replace d_crit_g2_rev1 = 1 if q4233==2 & q4225_causemed2!=1 & q4224==1

* # Revision 2:
gen d_crit_g2 = q4233!=1 & q4225_causemed2!=1
replace d_crit_g2 = . if q4233==.

tab d_crit_g2_rev1 d_crit_g2, missing
```

#### Criterio G3

```{stata criterion-g3-3}
*G3 criterion: "The episode is not attributable to psychoactive substance use or to any organic mental disorder"

* # Revision 1:
gen d_crit_g3_rev1 = .
replace d_crit_g3_rev1 = 0 if (q4224a==1 & q4224b==1) | (q4224==1 & (q4225_causemed12==1 | q4225_causemed23==1 | q4225_causemed24==1 | q4225_causemed25==1 | q4225_causemed30==1 | q4225_causemed31==1))
replace d_crit_g3_rev1 = 1 if (q4224a==2 | q4224b==2) | (q4224==1 & (q4225_causemed12!=1 & q4225_causemed23!=1 & q4225_causemed24!=1 & q4225_causemed25!=1 & q4225_causemed30!=1 & q4225_causemed31!=1))

* # Revision 2:
gen d_crit_g3 = (q4224a!=1 & q4224a!=.) | (q4224b!=1 & q4224b!=.) | (q4224==1 & (q4225_causemed12!=1 & q4225_causemed23!=1 & q4225_causemed24!=1 & q4225_causemed25!=1 & q4225_causemed30!=1 & q4225_causemed31!=1))
replace d_crit_g3 = . if q4224==. & q4224a==. & q4224b==.

tab d_crit_g3_rev1 d_crit_g3, missing
```
...
* # Revision 1:
gen d_crit_a_rev1 = .
replace d_crit_a_rev1 = 0 if d_crit_g1_rev1==0 | d_crit_g2_rev1==0 | d_crit_g3_rev1==0
replace d_crit_a_rev1 = 1 if d_crit_g1_rev1==1 & d_crit_g2_rev1==1 & d_crit_g3_rev1==1
...
tab d_crit_a_rev1 d_crit_a, missing
```
...
* # Revision 1:
gen d_b1_rev1 = .
replace d_b1_rev1 = 0 if (q4220a_alt==0 | q4220b_alt==0) & (q4220c_alt==0 | q4220d_alt==0)
replace d_b1_rev1 = 1 if q4220a_alt==1 | q4220b_alt==1 | q4220c_alt==1 | q4220d_alt==1

* # Revision 2:
gen d_b1 = q4220a==1 | q4220b==1 | q4220c==1 | q4220d==1
replace d_b1 = . if q4220a==.

tab d_b1_rev1 d_b1, missing
```

#### Criterio B2

```{stata criterion-b2-3}
*B2: "Loss of interest or pleasure in activities that are normally pleasurable"

* # Revision 1:
gen d_b2_rev1 = .
replace d_b2_rev1 = 0 if q4220e_alt==0 & q4220f_alt==0
replace d_b2_rev1 = 1 if q4220e_alt==1 | q4220f_alt==1

* # Revision 2:
gen d_b2 = q4220e==1 | q4220f==1
replace d_b2 = . if q4220e==.

tab d_b2_rev1 d_b2, missing
```

#### Criterio B3

```{stata criterion-b3-3}
*B3: "Decreased energy or increased fatigability"

* # Revision 1:
gen d_b3_rev1 = 0
replace d_b3_rev1 = 1 if q4222e_alt==1
replace d_b3_rev1 = . if q4222e_alt==.

* # Revision 2:
gen d_b3 = q4222e==1 if q4222e!=.

tab d_b3_rev1 d_b3, missing
```
...
egen d_screentotal_rev1 = rowtotal (d_b1_rev1 d_b2_rev1 d_b3_rev1), missing
egen d_screenmissing_rev1 = rowmiss (d_b1_rev1 d_b2_rev1 d_b3_rev1)

gen d_crit_b_rev1 = .
replace d_crit_b_rev1 = 0 if (d_screentotal_rev1 + d_screenmissing_rev1) < 2
replace d_crit_b_rev1 = 1 if d_screentotal_rev1 >= 2 & d_screentotal_rev1 != .
...
tab d_screentotal_rev1 d_screentotal, missing
tab d_crit_b_rev1 d_crit_b, missing
```
...
* # Revision 1:
gen d_c1_rev1 = .
replace d_c1_rev1 = 0 if q4222k_alt==0 & q4222l_alt==0
replace d_c1_rev1 = 1 if q4222k_alt==1 | q4222l_alt==1 | q4222m_alt==1

* # Revision 2:
gen d_c1 = q4222k==1 | q4222l==1 | q4222m==1
replace d_c1 = . if q4222k==. & q4222l==.

tab d_c1_rev1 d_c1, missing
```

#### Criterio C2

```{stata criterion-c2-3}
*C2: "Unreasonable feelings of self-reproach or excessive and inappropriate guilt"

* # Revision 1:
gen d_c2_rev1 = .
replace d_c2_rev1 = 0 if q4222n_alt==0
replace d_c2_rev1 = 1 if q4222n_alt==1

* # Revision 2:
gen d_c2 = q4222n==1 if q4222n!=.

tab d_c2_rev1 d_c2, missing
```

#### Criterio C3

```{stata criterion-c3-3}
*C3: "Recurrent thoughts of death or suicide, or any suicidal behaviour"

* # Revision 1:
gen d_c3_rev1 = .
replace d_c3_rev1 = 0 if q4222r_alt==0 & q4222s_alt==0 & q4222t_alt==0
replace d_c3_rev1 = 1 if q4222r_alt==1 | q4222s_alt==1 | q4222t_alt==1 | q4222u_alt==1 | q4222v_alt==1

* # Revision 2:
gen d_c3 = q4222r==1 | q4222s==1 | q4222t==1 | q4222u==1 | q4222v==1
replace d_c3 = . if q4222r==.

tab d_c3_rev1 d_c3, missing
```

#### Criterio C4

```{stata criterion-c4-3}
*C4: "Complaints or evidence of diminished ability to think or concentrate, such as indecisiveness or vacillation"

* # Revision 1:
gen d_c4_rev1 = .
replace d_c4_rev1 = 0 if q4222h_alt==0 & q4222i_alt==0 & q4222j_alt==0
replace d_c4_rev1 = 1 if q4222h_alt==1 | q4222i_alt==1 | q4222j_alt==1

* # Revision 2:
gen d_c4 = q4222h==1 | q4222i==1 | q4222j==1
replace d_c4 = . if q4222h==.

tab d_c4_rev1 d_c4, missing
```

#### Criterio C5

```{stata criterion-c5-3}
*C5: "Change in psychomotor activity, with agitation or retardation (either subjective or objective)"

* # Revision 1:
gen d_c5_rev1 = .
replace d_c5_rev1 = 0 if q4222f_alt==0 & q4222fa_alt==0 & q4222g_alt==0 & q4222ga_alt==0
replace d_c5_rev1 = 1 if (q4222f_alt==1 | q4222fa_alt==1) | (q4222g_alt==1 | q4222ga_alt==1)

* # Revision 2:
gen d_c5 = (q4222f==1 | q4222fa==1) | (q4222g==1 | q4222ga==1)
replace d_c5 = . if q4222f==.

tab d_c5_rev1 d_c5, missing
```

#### Criterio C6

```{stata criterion-c6-3}
*C6: "Sleep disturbance of any type"

* # Revision 1:
gen d_c6_rev1 = .
replace d_c6_rev1 = 0 if q4222c_alt==0 & q4222d_alt==0
replace d_c6_rev1 = 1 if q4222c_alt==1 | q4222d_alt==1

* # Revision 2:
gen d_c6 = q4222c==1 | q4222d==1
replace d_c6 = . if q4222c==.

tab d_c6_rev1 d_c6, missing
```

#### Criterio C7

```{stata criterion-c7-3}
*C7: "Change in appetite (decrease or increase) with corresponding weight change"

* # Revision 1:
gen d_c7_rev1 = .
replace d_c7_rev1 = 0 if q4222a_alt==0 | q4222b_alt==0
replace d_c7_rev1 = 1 if q4222a_alt==1 & q4222b_alt==1

* # Revision 2:
gen d_c7 = q4222a==1 & q4222b==1
replace d_c7 = . if q4222a==.

tab d_c7_rev1 d_c7, missing
```

#### Cálculo final del criterio C

```{stata criterion-symtotal-3}
*Sum of depressive symptoms

* # Revision 1:
egen d_symtotal_rev1 = rowtotal (d_c1_rev1 d_c2_rev1 d_c3_rev1 d_c4_rev1 d_c5_rev1 d_c6_rev1 d_c7_rev1), missing
egen d_symmissing_rev1 = rowmiss (d_c1_rev1 d_c2_rev1 d_c3_rev1 d_c4_rev1 d_c5_rev1 d_c6_rev1 d_c7_rev1)

* # Revision 2:
egen d_symtotal = rowtotal (d_c1 d_c2 d_c3 d_c4 d_c5 d_c6 d_c7), missing

tab d_symtotal_rev1 d_symtotal, missing
```
...
* ## Generate filter variables first:
gen  f4217_rev1     = q4214==1 & q4215==4 & q4216==4 if q4214!=.
egen f4221_sum_rev1 = rowtotal (q4220a_alt-q4220f_alt), missing
egen f4223_sum_rev1 = rowtotal (q4220a_alt-q4222v_alt), missing

* ## Compute the "skipped" variable:
gen     d_skipped_rev1 = .
replace d_skipped_rev1 =                     q4202!=1           if q4202!=.
replace d_skipped_rev1 = d_skipped_rev1==1 | q4211!=1           if q4211!=.
replace d_skipped_rev1 = d_skipped_rev1==1 | q4213!=1           if q4213!=.
replace d_skipped_rev1 = d_skipped_rev1==1 | q4218!=1           if q4218!=.
replace d_skipped_rev1 = d_skipped_rev1==1 | f4217==1           if f4217!=.
replace d_skipped_rev1 = d_skipped_rev1==1 | f4221_sum_rev1 < 1 if f4221_sum_rev1!=.
replace d_skipped_rev1 = d_skipped_rev1==1 | f4223_sum_rev1 < 2 if f4223_sum_rev1!=.
...
tab d_skipped_rev1 d_skipped, missing
```
...
*Total number of symptoms (B + C)
gen     d_lifetime_severity_total_rev1 = .
replace d_lifetime_severity_total_rev1 = 0 if d_screentotal_rev1 + d_screenmissing_rev1 + d_symtotal_rev1 + d_symmissing_rev1 < 4
replace d_lifetime_severity_total_rev1 = 1 if d_screentotal_rev1 + d_symtotal_rev1 >= 4 & d_screentotal_rev1!=. & d_symtotal_rev1 !=.
replace d_lifetime_severity_total_rev1 = 2 if d_screentotal_rev1 + d_symtotal_rev1 >= 6 & d_screentotal_rev1!=. & d_symtotal_rev1 !=.
replace d_lifetime_severity_total_rev1 = 3 if d_screentotal_rev1 + d_symtotal_rev1 >= 8 & d_screentotal_rev1!=. & d_symtotal_rev1 !=.

*CIDI initial screening test
gen d_screening_rev1=.
replace d_screening_rev1=0 if q4200==2 & q4201==2 & q4202==2
replace d_screening_rev1=1 if q4200==1 | q4201==1 | q4202==1

gen     d_lifetime_severity_rev1 = .
replace d_lifetime_severity_rev1 = 0 if d_screening_rev1==0
replace d_lifetime_severity_rev1 = 0 if d_screening_rev1==1 & d_crit_a_rev1==0 | d_crit_b_rev1==0      | d_lifetime_severity_total_rev1==0
replace d_lifetime_severity_rev1 = 1 if d_screening_rev1==1 & d_crit_a_rev1==1 & d_crit_b_rev1==1      & d_lifetime_severity_total_rev1>=1 & d_lifetime_severity_total!=.
replace d_lifetime_severity_rev1 = 2 if d_screening_rev1==1 & d_crit_a_rev1==1 & d_crit_b_rev1==1      & d_lifetime_severity_total_rev1>=2 & d_lifetime_severity_total!=.
replace d_lifetime_severity_rev1 = 3 if d_screening_rev1==1 & d_crit_a_rev1==1 & d_screentotal_rev1==3 & d_lifetime_severity_total_rev1==3
...
tab d_lifetime_severity_rev1 d_lifetime_severity, missing
```

### Diagnóstico final de depresión

```{stata liferime-depression-3}
* # Revision 1:
gen     depression_lifetime_rev1 = .
replace depression_lifetime_rev1 = 0 if d_lifetime_severity_rev1==0
replace depression_lifetime_rev1 = 1 if d_lifetime_severity_rev1>=1 & d_lifetime_severity_rev1 !=.
...
tab depression_lifetime_rev1 depression_lifetime, missing
```

## Sintaxis completa (Stata 13)

```{stata reload-data-4, include=FALSE}
use "~\UAM\marta.miret@uam.es - Bases de datos maestras Edad con Salud\Ola_3\Cohorte_2019\rawdata_c2019w1.dta", clear
```

A continuación aparecen en orden inverso (la versión "conservadora" después)
para evitar que la recodificación de los ítems afecte a la otra versión.

### Versión "liberal (específica)"

```{stata review-1-comp, results='hide'}
***Depressive episode (F32)
...
***Lifetime depression diagnostic
gen     depression_lifetime_rev2 = .
replace depression_lifetime_rev2 = 0 if d_lifetime_severity==0
replace depression_lifetime_rev2 = 1 if d_lifetime_severity>=1 & d_lifetime_severity !=.
*label define yesno 0 "No" 1 "Yes"
label values   depression_lifetime_rev2 yesno
label variable depression_lifetime_rev2 "Lifetime depression (c2019_w1)"
```

### Versión "conservadora"

```{stata, include=FALSE}
drop d_crit_g1-d_lifetime_severity
```

```{stata review-2-comp, results='hide'}
*Previous recoding of the symptomatology questions
recode q4220a-q4222v (1=1) (2=0) (888 999=.) 
...
***Lifetime depression diagnostic
gen     depression_lifetime_rev1 = .
replace depression_lifetime_rev1 = 0 if d_lifetime_severity==0
replace depression_lifetime_rev1 = 1 if d_lifetime_severity>=1 & d_lifetime_severity !=.
*label define yesno 0 "No" 1 "Yes"
label values   depression_lifetime_rev1 yesno
label variable depression_lifetime_rev1 "Lifetime depression (c2019_w1)"
```

### Comparación de resultados

#### Contingencia entre los resultados de ambas revisiones

```{stata contingency-final-3}
tab depression_lifetime_rev1 depression_lifetime_rev2, missing
```
...
```{stata freqs-3}
tab depression_lifetime_rev1
tab depression_lifetime_rev2
```
